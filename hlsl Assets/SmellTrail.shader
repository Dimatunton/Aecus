Shader "Custom/SmellTrail"
{
    Properties
    {
        ...
        _VertexSwayUseNoiseTex ("Vertex Sway Use NoiseTex (0=FBM,1=Tex)", Float) = 1
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // lightweight value-noise (quintic interpolation) - unchanged, still cheap relative to many texture ops
            ...
            // Optimized FBM: fewer octaves, precomputed rotation terms, faster amplitude falloff
            static float fbm_noise_fast(float2 p)
            {
                float sum = 0.0;
                float amp = 0.75;
                float freq = 1.0;

                // Precomputed cos/sin for angles 0, ~0.314159, ~0.628318 (cheap rotation spread)
                const float cosA0 = 1.0, sinA0 = 0.0;
                const float cosA1 = 0.9510565163, sinA1 = 0.3090169944;
                const float cosA2 = 0.8090169944, sinA2 = 0.5877852523;

                // 3 octaves is usually enough for smooth sway and much cheaper than 5
                {
                    float2 rp = float2(p.x * cosA0 - p.y * sinA0, p.x * sinA0 + p.y * cosA0);
                    sum += amp * valueNoise(rp * freq);
                    freq *= 2.0;
                    amp *= 0.5;
                }
                {
                    float2 rp = float2(p.x * cosA1 - p.y * sinA1, p.x * sinA1 + p.y * cosA1);
                    sum += amp * valueNoise(rp * freq);
                    freq *= 2.0;
                    amp *= 0.5;
                }
                {
                    float2 rp = float2(p.x * cosA2 - p.y * sinA2, p.x * sinA2 + p.y * cosA2);
                    sum += amp * valueNoise(rp * freq);
                }
            ...
            // multi-harmonic traveling wave (small loop)
            ...
            v2f vert(appdata_t v)
            {
                ...
                // FBM-based sway sampling in world space with cheaper temporal low-pass:
                float t = _Time.y * _Speed;
                float2 nPos = worldPosBefore.xz * (_NoiseScale * _SwayFrequency);

                // Use a texture-based vertex sway (cheap) if requested, otherwise use optimized FBM
                float n = 0.5;
                if (_VertexSwayUseNoiseTex > 0.5)
                {
                    // two texture samples with small time-offset for cheap decorrelation
                    float n0 = tex2D(_NoiseTex, nPos + float2(t * 0.3, t * 0.3)).r;
                    float n1 = tex2D(_NoiseTex, nPos + float2((t + 0.47) * 0.3, (t + 0.47) * 0.3)).r;
                    float raw = n0 * 0.7 + n1 * 0.3;
                    n = lerp(n0, raw, clamp(_TemporalBlend, 0.0, 1.0));
                }
                else
                {
                    // optimized procedural FBM with fewer octaves and fewer trig ops
                    float n0 = fbm_noise_fast(nPos + float2(t, t));
                    float n1 = fbm_noise_fast(nPos + float2(t + 0.37, t + 0.37));
                    float raw = n0 * 0.6 + n1 * 0.4;
                    n = lerp(n0, raw, clamp(_TemporalBlend, 0.0, 1.0));
                }
                float n01 = (n - 0.5) * 2.0;
            ...
            {
                // use half precision in the fragment stage where possible
                half2 baseUV;
                if (_MainTexUseWorld > 0.5)
                {
                    baseUV = (half2)(i.worldPos.xz * _MainTexWorldScale);
                }
                else
                {
                    baseUV = (half2)(i.uv * _MainTexScale.xy);
                }

                // texture and tint (keep tex fetches, but minimize extra math)
                fixed4 baseTex = tex2D(_MainTex, baseUV);
                fixed3 baseColor = baseTex.rgb * _Color.rgb;
                half baseAlpha = baseTex.a * _Color.a;

                // fragment noise for subtle color variation
                half2 noiseUV = (half2)(i.worldPos.xz * _NoiseScale + float2(_Time.y * _Speed, _Time.y * _Speed));
                half n = tex2D(_NoiseTex, noiseUV * _SwayFrequency).r;
                half nScaled = pow(saturate(n * saturate(_NoiseStrength)), 1.25);

                // alpha falloff at both head and tail:
                half head = pow(saturate(i.progress), max(0.0001, _HeadFalloff));
                half tail = pow(1.0 - saturate(i.progress), max(0.0001, _AlphaFalloff));
                half alphaFromProgress = head * tail;

                half edge = smoothstep(0.0, _EdgeSoftness, i.uv.y) * (1.0 - smoothstep(1.0 - _EdgeSoftness, 1.0, i.uv.y));
                half noiseAlpha = lerp(0.95, 1.05, nScaled);

                half alpha = baseAlpha * alphaFromProgress * edge * noiseAlpha * i.vcol.a;
